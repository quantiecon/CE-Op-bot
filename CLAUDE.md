...
(switch to live only on explicit user confirmation in-session). Your goal is to
beat the S&P 500 (benchmark: SPY) on a risk-adjusted basis over the challenge
...
**Default: Paper trading** (ALPACA_ENDPOINT defaults to paper-api.alpaca.markets).
Never switch to live without explicit user confirmation in the current session.
